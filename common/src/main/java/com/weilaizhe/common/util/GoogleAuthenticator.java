...
public class GoogleAuthenticator {
    ...
    /**
     * @author: dameizi
     * @dateTime: 2019-05-27 14:21
     * @description: 验证动态口令
     * @param: [request, response, securityCode, secret]
     * @return: java.lang.Object
     */
    public static Object getAuthenticator(HttpServletRequest request, HttpServletResponse response, String securityCode, String secret) {
        /*if (!securityCode.matches("[0-9]{6}")){
            return MessageFormat.format(CommonConst.ERROR, CommonConst.DYNAMIC_PASSWORD_ERROR);
        }redissonClient.getFairLock
        Boolean authcode = authcode(securityCode, secret);
        // 动态口令正确才返回头部信息
        if(authcode){
            ...
            return MessageFormat.format(CommonConst.ERROR, CommonConst.DYNAMIC_PASSWORD_PAST);
        }*/
        //设置响应头信息，告诉浏览器不要缓存此内容
        response.setHeader("Pragma", "No-cache");
        response.setHeader("Cache-Control", "no-cache");
